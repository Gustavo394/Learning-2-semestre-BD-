...
public class Viagem{
    ...
    void salvar(ActionEvent event){
        ...
        try{
            if (cpf_viajante_final.getValue() == null || placa_veiculo_final.getValue() == null || cep_destino_final.getValue() == null ||
            ...
            } else if (!partida.isAfter(atual.plusDays(0))){
                cadastro_viagem.setDisable(true);
                telaAviso.titulo = "Alerta";
                telaAviso.texto = "A data de partida deve ser no mínimo\num dia após a data atual!";
                telaAviso telaAviso = new telaAviso();
                telaAviso.aviso();
                cadastro_viagem.setDisable(false);
                ...
                cadastro_viagem.setDisable(true);
                telaAviso.titulo = "Alerta";
                telaAviso.texto = "A data de retorno deve ser no mínimo\num dia após a data de partida!";
                telaAviso telaAviso = new telaAviso();
                telaAviso.aviso();
                cadastro_viagem.setDisable(false);
            } else {
                    ...
                    PreparedStatement viagemcpf = conexao.prepareStatement("SELECT * FROM viagem WHERE cpf_viajante = ?");
                    viagemcpf.setString(1, cpf_viajante_final.getValue());
                    ResultSet viagemcpfrs = viagemcpf.executeQuery();
                    
                    PreparedStatement viagemplaca = conexao.prepareStatement("SELECT * FROM viagem WHERE placa_veiculo = ?");
                    viagemplaca.setString(1, placa_veiculo_final.getValue());
                    ResultSet viagemplacars = viagemplaca.executeQuery();

                    if (viagemcpfrs.next()){
                        cadastro_viagem.setDisable(true);
                        telaAviso.titulo = "Alerta";
                        telaAviso.texto = "Esta pessoa já está registrada em uma viagem!";
                        telaAviso telaAviso = new telaAviso();
                        telaAviso.aviso();
                        cadastro_viagem.setDisable(false);
                        
                    } else if (viagemplacars.next()){
                        cadastro_viagem.setDisable(true);
                        telaAviso.titulo = "Alerta";
                        telaAviso.texto = "Este veículo já está registrada em uma viagem!";
                        telaAviso telaAviso = new telaAviso();
                        telaAviso.aviso();
                        cadastro_viagem.setDisable(false);
